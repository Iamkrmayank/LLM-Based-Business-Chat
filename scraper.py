...
def main():
    ...
    # Ask user for search location
    search_location = input("Enter the location to search (e.g., 'Delhi, Delhi' or 'New York, NY'): ").strip()
    if not search_location:
        print("No location provided. Exiting.")
        return

    # Split the location into city and state (optional, for display purposes)
    location_parts = [part.strip() for part in search_location.split(',')]
    selected_city = location_parts[0]
    selected_state = location_parts[1] if len(location_parts) > 1 else ""
    ...
    with sync_playwright() as p:
        ...
        for selected_business_type in selected_business_types:
            ...
            print(f"Searching for {selected_business_type} in {search_location}.")
            search_for = f"{selected_business_type} in {search_location}"
            ...
            try:
                ...
                print(f"Timeout error occurred while searching for {selected_business_type} in {search_location}: {e}")
                continue
            except Exception as e:
                print(f"Error occurred while searching for {selected_business_type} in {search_location}: {e}")
                continue

            try:
                current_count = page.locator('//a[contains(@href, "https://www.google.com/maps/place")]').count()
            except Exception as e:
                print(f"Error detecting results for {search_location}, skipping: {e}")
                continue

            if current_count == 0:
                print(f"No results found for {selected_business_type} in {search_location}.")
                continue

            print(f"Found {current_count} listings for {selected_business_type} in {search_location}.")
            ...
            while listings_scraped < num_listings_to_capture:
                ...
                if new_count == previously_counted:
                    ...
                    if scroll_attempts >= MAX_SCROLL_ATTEMPTS:
                        print(f"No more listings found after {scroll_attempts} scroll attempts.")
                        break
                else:
                    scroll_attempts = 0
                ...
                if page.locator("text=You've reached the end of the list").is_visible():
                    print(f"Reached the end of the list in {search_location}.")
                    break

            # Save any remaining businesses after finishing this business type
